...
class PermitActivityAgent:
    def __init__(self, agent_id: str = None):
        self.agent_id = agent_id or _as["permit_activity_id"]
        self.name = C.AGENT_PERMIT_ACTIVITY
        self.status = "idle"
    ...
    def analyze(self, state: Dict) -> Dict:
        ...
        if not permits:
            ...
            return findings
        for p in permits:
            ptype = p.get("type", C.UNKNOWN_LABEL)
            zone_id = p.get("zone_id", C.UNKNOWN_LABEL)
            ...
            if risk in C.HIGH_RISK_LEVELS:
                findings["high_risk_permits"].append({
                    "id": p.get("id", ""), "type": ptype, "zone_id": zone_id,
                    "zone_name": p.get("zone_name", ""), "risk_level": risk,
                    "workers": p.get("workers", []), "description": p.get("description", ""),
                })
        for zid, count in findings["permits_by_zone"].items():
            if count > 1:
                zone_permits = [p for p in permits if p.get("zone_id") == zid]
                types = [p.get("type", C.UNKNOWN_LABEL) for p in zone_permits]
                findings["overlapping_zone_permits"].append({
                    "zone_id": zid, "zone_name": zone_permits[0].get("zone_name", ""),
                    "permit_count": count, "permit_types": types,
