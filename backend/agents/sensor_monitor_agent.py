...
class SensorMonitorAgent:
    def __init__(self, agent_id: str = "sensor_monitor_1"):
        self.agent_id = agent_id
        self.name = "Sensor Monitor Agent"
        self.status = "idle"
    ...
    def analyze(self, state: Dict) -> Dict:
        ...
        for sid, s in sensors.items():
            zone_id = s.get("zone_id", "unknown")
            if zone_id not in zone_sensors:
                zone_sensors[zone_id] = {"total": 0, "critical": 0, "warning": 0, "normal": 0, "details": []}
        ...
        for zid, zdata in zone_sensors.items():
            ...
            findings["zone_alerts"][zid] = {
                "severity": severity,
                "total_sensors": zdata["total"],
                "critical_count": zdata["critical"],
                "warning_count": zdata["warning"],
