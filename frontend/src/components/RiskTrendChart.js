    ...
    <div style={{ padding: '8px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: COLORS.textMuted }}>Last {data.length} readings · 2s intervals</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 20, fontWeight: 700, color }}>{(currentScore * 100).toFixed(0)}</div>
          <div style={{ fontSize: 10, color: trend > 0.05 ? COLORS.riskCritical : trend < -0.05 ? COLORS.riskNormal : COLORS.textMuted }}>
            {trend > 0.05 ? `\u{2191} Rising` : trend < -0.05 ? `\u{2193} Falling` : '\u{2192} Stable'}
          </div>
        </div>
      </div>
      <svg width={width} height={height} style={{ background: COLORS.bgModal, borderRadius: 8 }}>
        <defs>
          <linearGradient id={`riskGradient-${uid}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={color} stopOpacity="0.25" />
            <stop offset="100%" stopColor={color} stopOpacity="0.01" />
          </linearGradient>
        </defs>
        <polyline points={points} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        <polygon points={`${points} ${padding + chartW},${padding + chartH} ${padding},${padding + chartH}`} fill={`url(#riskGradient-${uid})`} />
        <circle cx={scores.length > 1 ? padding + ((scores.length - 1) / Math.max(scores.length - 1, 1)) * chartW : padding}
                ...
                r="3" fill={color} stroke={COLORS.bgModal} strokeWidth="1.5" />
        <text x={padding} y={padding - 2} fontSize="9" fill={COLORS.textDim}>{(maxScore * 100).toFixed(0)}</text>
        <text x={padding} y={padding + chartH + 10} fontSize="9" fill={COLORS.textDim}>{(minScore * 100).toFixed(0)}</text>
      </svg>
    </div>
  );
}
